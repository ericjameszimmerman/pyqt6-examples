...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # Setup the sidebar within its own scroll area
        sidebar = QVBoxLayout()
        scroll = QScrollArea()
        ...
        for i in range(1, 33):
            ...
            toggleButton.toggled.connect(lambda checked, topic=str(i), btn=toggleButton: self.toggle_topic(topic, checked, btn))
            topicLayout.addWidget(toggleButton)

            eyeButton = QPushButton()
            eyeButton.setIcon(eye_open_icon)
            eyeButton.clicked.connect(lambda _, topic=str(i), btn=eyeButton: self.toggle_visibility(topic, btn, eye_open_icon, eye_closed_icon))
    ...
    def toggle_topic(self, topic, checked, button):
        if checked:
